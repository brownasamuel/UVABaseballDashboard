...
raw_text <- map("https://static.virginiasports.com/custompages/sports/m-basebl/stats/2022-2023/SeasonStats.pdf?_gl=1*11rpgpg*_ga*MTg1NjExNzA0NC4xNjgwMTg0ODQy*_ga_X9RVV1P9QW*MTY4MTkyNDQ1Ni4xMC4xLjE2ODE5MjQ4NDUuNjAuMC4w&_ga=2.154379455.640199771.1681924456-1856117044.1680184842", 
                pdf_text)[[1]]
pit_stats <- str_split(raw_text, "\n")[[1]][which.max(str_detect(str_split(raw_text, "\n")[[1]], "b/avg")) : 
                                 max(which(str_detect(str_split(raw_text, "\n")[[1]], "^[0-9]")))] %>%
  str_squish() %>%
  sub(".*?\\d ", "", x = .) %>%
  sub(" ", "", x = .) %>%
  ...
  drop_na(era) %>%
  mutate(Init = sapply(str_extract_all(Player, "[:upper:]"), function(x) x[2]),
         Player = str_replace(Player, Init, paste(" ", Init, sep = "")),
...
# Player tab function
pitcherTab <- function(name){
  tabPanel(title = name, value = name,
           fluidRow(
             p(name, style = "text-align: center; font-weight: bold; font-size: 170%; 
               padding-top: 15px; padding-bottom: 10px"),
             column(2, div(imageOutput(paste0("pic_", paste0(unlist(str_extract_all(name, "[:upper:]")), 
                                                             collapse = ""))), style = "height:294px")),
             column(9, div(dataTableOutput(paste0("stats_", paste0(unlist(str_extract_all(name, "[:upper:]")), 
                                                                   collapse = ""))), style = "font-size: 95%"))),
           # Put in options for what to look at
           tabsetPanel(tabPanel(title = "Overview",
                                # Add in baseball savant type page
                                ),
                       ...
                       tabPanel(title = "Game Summaries",
                                sidebarLayout(
                                  sidebarPanel(width = 4,
                                               # Choose opponents
                                               uiOutput(paste0(
                                                 "Opponent", paste0(unlist(str_extract_all(name,"[:upper:]")), 
                                                                    collapse = ""), "gs")),
                                               # Choose pitch types
                                               uiOutput(paste0(
                                                 "pType", paste0(unlist(str_extract_all(name,"[:upper:]")), 
                                                                    collapse = ""), "gs")),
                                               # Choose batter handedness
                                               uiOutput(paste0(
                                                 "bHand", paste0(unlist(str_extract_all(name,"[:upper:]")), 
                                                                    collapse = ""), "gs")),
                                               # Choose outcomes
                                               uiOutput(paste0(
                                                 "Outcome", paste0(unlist(str_extract_all(name,"[:upper:]")), 
                                                                    collapse = ""), "gs"))
                                               ),
                                  
                                  
                                  mainPanel(width = 8, 
                                            div(plotOutput(paste0("pitchZone", paste0(unlist(str_extract_all(name, "[:upper:]")), collapse = ""), "gs"))))),
                                  
                                  fluidRow(
                                    column(12,
                                           div(DT::dataTableOutput(paste0("pitchTable", paste0(unlist(str_extract_all(name, "[:upper:]")), collapse = ""), "gs")), 
                       ...
                       tabPanel(title = "Scatter Plots",
                                ...
                                fluidRow(column(2, uiOutput(paste0(
                                  ...
                                  column(6, plotOutput(paste0("sp_plot_", 
                                                              paste0(unlist(str_extract_all(name,"[:upper:]")), 
                                                                     collapse = "")))),
                                  column(4, 
                                         div(dataTableOutput(paste0("sp_table_", 
                                                                    paste0(unlist(str_extract_all(name,"[:upper:]")),
                                                                           collapse = ""))),
                                             style = "font_size: 80%")))
                                ),
...
ui <- fluidPage(
  ...
  navbarPage(
    ...
    navbarMenu(title = "Pitcher Profiles",
               ...
               pitcherTab("Jake Berry"),
               pitcherTab("Evan Blanco"),
               pitcherTab("Matthew Buchanan"),
               pitcherTab("Connelly Early"),
               pitcherTab("Brian Edgington"),
               pitcherTab("Alex Greene"),
               pitcherTab("Bradley Hodges"),
               pitcherTab("Jake Hodorovich"),
               pitcherTab("Chase Hungate"),
               pitcherTab("Kevin Jaxel"),
               pitcherTab("Avery Mabe"),
               pitcherTab("Cullen McKay"),
               pitcherTab("Jack O'Connor"),
               pitcherTab("Nick Parker"),
               pitcherTab("Angelo Tonas"),
               pitcherTab("Jay Woolfolk")
               )
    )
  )
...
server = function(input, output) {
  ...
  ##########################
  ### Inner tab 2 - Game Summaries
  # Select opponent
  observe({
    output[[
        paste0("Opponent", paste0(unlist(str_extract_all(input$Dash, "[:upper:]")), collapse = ""), "gs")]] <- renderUI({
          pickerInput(paste0("opponent", paste0(unlist(str_extract_all(input$Dash, "[:upper:]")), collapse = ""), "gs"), 
                      "Game", 
                      choices = c(virginia %>% 
  ...
  observe({
    output[[
        paste0("pType", paste0(unlist(str_extract_all(input$Dash, "[:upper:]")), collapse = ""), "gs")]] <- renderUI({
          pickerInput(paste0("ptype", paste0(unlist(str_extract_all(input$Dash, "[:upper:]")), collapse = ""), "gs"), 
                      "Pitch Type", 
                      choices = virginia %>% 
                        filter(Pitcher %in% sub("([^ ]*) ([^ ]*)", "\\2, \\1", input$Dash),
                               opponent %in% input[[paste0("opponent", 
                                                           paste0(unlist(str_extract_all(input$Dash, "[:upper:]")), collapse = ""), "gs")]]) %>% 
                        pull(TaggedPitchType) %>% 
                        unique(), 
                      selected = virginia %>% 
                        filter(Pitcher %in% sub("([^ ]*) ([^ ]*)", "\\2, \\1", input$Dash),
  ...
  observe({
    output[[
        paste0("bHand", paste0(unlist(str_extract_all(input$Dash, "[:upper:]")), collapse = ""), "gs")]] <- renderUI({
    pickerInput(paste0("bhand", paste0(unlist(str_extract_all(input$Dash, "[:upper:]")), collapse = ""), "gs"), 
                "Batter handedness", 
                choices = c("Right", "Left"), 
                selected = c("Right", "Left"),
                options = list('actions-box' = TRUE),
  ...
  observe({
    output[[
        paste0("Outcome", paste0(unlist(str_extract_all(input$Dash, "[:upper:]")), collapse = ""), "gs")]] <- renderUI({
    pickerInput(paste0("outcome", paste0(unlist(str_extract_all(input$Dash, "[:upper:]")), collapse = ""), "gs"), 
                "Outcome", 
                choices = virginia %>% 
                  filter(Pitcher %in% sub("([^ ]*) ([^ ]*)", "\\2, \\1", input$Dash),
                          opponent %in% input[[paste0("opponent", paste0(unlist(str_extract_all(input$Dash, "[:upper:]")), collapse = ""), "gs")]],
                          TaggedPitchType %in% input[[paste0("ptype", paste0(unlist(str_extract_all(input$Dash, "[:upper:]")), collapse = ""), "gs")]]) %>% 
                   mutate(PitchCall = case_when(
                     PitchCall == "InPlay" ~ PlayResult,
                     PitchCall != "InPlay" ~ PitchCall)) %>%
                   pull(PitchCall) %>% 
  ...
  # Make the zone plot
  observe({
    output[[paste0("pitchZone", paste0(unlist(str_extract_all(input$Dash, "[:upper:]")), collapse = ""), "gs")]] <- renderPlot({
      virginia %>%
          mutate(PitchCall = case_when(
            PlayResult == "Single" | PlayResult == "Double" | PlayResult == "Triple" ~ "Hit",
            PlayResult == "Out" | PlayResult == "FieldersChoice" | PlayResult == "Error" ~ "Out",
          ...
          filter(PitchCall != "",
                 ...
                 opponent %in% input[[paste0("opponent", paste0(unlist(str_extract_all(input$Dash, "[:upper:]")), collapse = ""), "gs")]],
                 TaggedPitchType %in% input[[paste0("ptype", paste0(unlist(str_extract_all(input$Dash, "[:upper:]")), collapse = ""), "gs")]],
                 PitchCall %in% input[[paste0("outcome", paste0(unlist(str_extract_all(input$Dash, "[:upper:]")), collapse = ""), "gs")]],
                 BatterSide %in% input[[paste0("bhand", paste0(unlist(str_extract_all(input$Dash, "[:upper:]")), collapse = ""), "gs")]]) %>%
          ggplot(aes(x = PlateLocSide, y = PlateLocHeight)) +
          geom_point(aes(color = PitchCall), na.rm = TRUE) +
          xlim(-2.5, 2.5) +
          theme_bw() +
  ...
  # Pitch results
  observe({
    output[[paste0("pitchTable", paste0(unlist(str_extract_all(input$Dash, "[:upper:]")), collapse = ""), "gs")]] <- DT::renderDataTable({
      datatable(virginia %>%
                  filter(PitchCall != "",
                         Pitcher %in% sub("([^ ]*) ([^ ]*)", "\\2, \\1", input$Dash),
                         opponent %in% input[[paste0("opponent", paste0(unlist(str_extract_all(input$Dash, "[:upper:]")), collapse = ""), "gs")]],
                         TaggedPitchType %in% input[[paste0("ptype", paste0(unlist(str_extract_all(input$Dash, "[:upper:]")), collapse = ""), "gs")]],
                         BatterSide %in% input[[paste0("bhand", paste0(unlist(str_extract_all(input$Dash, "[:upper:]")), collapse = ""), "gs")]]) %>%
                  group_by(Pitcher, TaggedPitchType) %>%
  ...
  # Make the game-to-game graph
  observe({
    output[[paste0("gtg_graph_", 
                   paste0(unlist(str_extract_all(input$Dash, "[:upper:]")), collapse = ""))]] <- renderPlot({
                     allGames %>%
                       filter(Pitcher == sub("([^ ]*) ([^ ]*)", "\\2, \\1", input$Dash),
                              TaggedPitchType %in% input[[paste0("ptype", 
                                                                 paste0(unlist(str_extract_all(input$Dash, "[:upper:]")), collapse = ""),
                                                                 "gtg")]]) %>%
                       ...
                       ggplot(aes(x = opponent, 
                                  y = !!sym(input[[paste0("metric", 
                                                    paste0(unlist(str_extract_all(input$Dash, "[:upper:]")), 
                                                           collapse = ""), "gtg")]]),
                                  color = TaggedPitchType
                                  )) +
                       geom_point() +
                       geom_line(aes(group = TaggedPitchType)) +
                       theme_bw() + 
    ...
    output[[paste0("gtg_table_", 
                   paste0(unlist(str_extract_all(input$Dash, "[:upper:]")), collapse = ""))]] <- renderDataTable({
                     allGames %>%
                       ...
                       ungroup() %>%
                       filter(TaggedPitchType %in% input[[paste0("ptype", 
                                                               paste0(unlist(str_extract_all(input$Dash, "[:upper:]")), 
                                                                      collapse = ""), "gtg")]]) %>%
                       arrange(mdy(Date)) %>%
                       select(opponent, ct, TaggedPitchType,
                              input[[paste0("metric", 
                                            paste0(unlist(str_extract_all(input$Dash, "[:upper:]")), 
                                                   collapse = ""), "gtg")]]) %>%
      ...
      output[[paste0("tab_moveout_", 
                     paste0(unlist(str_extract_all(input$Dash, "[:upper:]")), collapse = ""))]] <- renderDataTable({
                       allGames %>%
                         ...
                         'Spin Rate' = round(mean(SpinRate, na.rm = T), 2),
                         ...
                         rename('Pitch Type' = TaggedPitchType, "Percentage Thrown" = Usage) %>%
                         select(-c(Pitcher, Strikes, Balls, Swings, BIP, Fouls, SwStr)) %>%
                         datatable(container = withTags(table(
                           class = 'display',
                           thead(
                             tr(
                               th(colspan = 2, ''),
                               th(colspan = 8, 'Season Averages'),
                             ...
                             tr(th(""), th("Pitch Type", style = "border-right: solid 2px;"),
                                th("Count"), th("Percentage Thrown"), th("Velo"), th("Vertical Break"), 
                                th("Horizontal Break"), th("Vertical Release"), 
                                th("Horizontal Release"), th("Spin Rate", style = "border-right: solid 2px;"), 
                         ...
                         formatStyle(c(1, 9, 13, 15, 19), `border-right` = "solid 2px")
                       })
      })
    # Usage table
    observe({
...
# Tab ideas:
# Game summaries(Make it plotly interactive?, also add colors for percentiles of metrics), player profiles(Add pitch tunneling?), pitcher comps, add in hitters eventually
