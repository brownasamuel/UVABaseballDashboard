              ...
              tabPanel(title = "Connelly Early", value = "Connelly Early",
                       ...
                       tabsetPanel(tabPanel(title = "In-Game Durability", value = "durab",
                                   ...
                                   tabPanel(title = "Game-to-game Performance",
                                            fluidRow(column(6, plotOutput("gtg_graph_CE")),
                                                     column(6, 
                                            ...
                                            fluidRow(column(6, plotOutput("pc_graph_KJ")),
                                                     column(6, 
                                                            div(dataTableOutput("pc_table_KJ"), 
                                                                style = "font-size: 80%")))),
                                   tabPanel(title = "Game-to-game Performance",
                                            fluidRow(column(6, plotOutput("gtg_graph_KJ")),
                                                     column(6, 
      ...
      output[[paste0("gtg_graph_", 
                     paste0(unlist(str_extract_all(input$Dash, "[:upper:]")), collapse = ""))]] <- renderPlot({
                       allGames %>%
                         filter(TaggedPitchType == "Fastball", 
                                Pitcher == sub("([^ ]*) ([^ ]*)", "\\2, \\1", input$Dash)) %>%
                         group_by(Pitcher, Date, opponent) %>%
                         summarize(velo = mean(RelSpeed), 
                                   .groups = "keep") %>%
                         ungroup() %>%
                         mutate(opponent = factor(opponent, levels = opponent)) %>%
                         ggplot(aes(x = opponent, y = velo)) +
                         geom_point() +
                         geom_line(aes(group = 1)) +
                         theme_bw() + 
                         theme(axis.text.x = element_text(angle = 15, hjust = .5, vjust = .5)) +
                         labs(title = paste(sub("([^ ]*) ([^ ]*)", "\\2, \\1", input$Dash), 
                                               "Average Velo by pitch count"), 
                              x = "Opponent", y = "Fastball Velo")
                       })
      })
    # Game-to-game table
    observe({
                     ...
                     paste0(unlist(str_extract_all(input$Dash, "[:upper:]")), collapse = ""))]] <- renderDataTable({
                       allGames %>%
                         filter(TaggedPitchType == "Fastball", 
                                 Pitcher == sub("([^ ]*) ([^ ]*)", "\\2, \\1", input$Dash)) %>%
                          group_by(Pitcher, Date, opponent) %>%
                          summarize(velo = round(mean(RelSpeed), 1), 
                                    .groups = "keep") %>%
