    ...
    navbarMenu(title = "Pitcher Profiles",
               ...
               tabPanel(title = "Connelly Early", value = "Connelly Early",
                        ...
                        tabsetPanel(tabPanel(title = "Overview",
                                    ...
                                    tabPanel(title = "Scatter Plots",
                                             ...
                                             fluidRow(column(2, uiOutput("xaxisCEsp"), uiOutput("yaxisCEsp")),
                                                      column(6, plotOutput("sp_plot_CE")),
                                                      column(4, div(dataTableOutput("sp_table_CE"),
                                                                    style = "font_size: 80%")))),
                                    ...
                                    tabPanel(title = "Tables",
                                             ...
                                             fluidRow(p(""),
                                                      div(dataTableOutput("tab_moveout_CE"), 
                                                          style = "font-size:90%")),
                                             fluidRow(p("Usage Table"),
                                                      div(dataTableOutput("tab_usage_CE"), 
                                                          style = "font-size:90%"))),
                                    ...
                                    tabPanel(title = "Game-to-game Performance",
                                             fluidRow(column(2, uiOutput("ptypeCEgtg"), uiOutput("metricCEgtg")),
                                                      ...
                                                      column(4, div(dataTableOutput("gtg_table_CE"), 
                                                                    style = "font-size: 80%"))))
                                    )
                        ),
               tabPanel(title = "Kevin Jaxel", value = "Kevin Jaxel",
                        fluidRow(
                          ...
                          column(9,
                                 div(dataTableOutput("stats_KJ"), style = "font-size: 95%"))),
                        tabsetPanel(tabPanel(title = "Overview",
                                             # Add in brooks baseball summary type thing
                                             ),
                                    tabPanel(title = "Scatter Plots",
                                             # Add in pie charts plus table of results
                                             fluidRow(column(2, uiOutput("xaxisKJsp"), uiOutput("yaxisKJsp")),
                                                      column(6, plotOutput("sp_plot_KJ")),
                                                      column(4, div(dataTableOutput("sp_table_KJ"),
                                                                    style = "font_size: 80%")))),
                                    tabPanel(title = "Zone Profile",
                                             # Add in counts
                                    ...
                                    tabPanel(title = "Tables",
                                             # Add in batter handedness that applies to both
                                             fluidRow(p(""),
                                                      div(dataTableOutput("tab_moveout_KJ"), 
                                                          style = "font-size:90%")),
                                             fluidRow(p("Usage Table"),
                                                      div(dataTableOutput("tab_usage_KJ"), 
                                                          style = "font-size:90%"))),
                                    tabPanel(title = "In-Game Durability", value = "durab",
                                    ...
                                    tabPanel(title = "Game-to-game Performance",
                                             fluidRow(column(2, uiOutput("ptypeKJgtg"), uiOutput("metricKJgtg")),
                                                      ...
                                                      column(4, div(dataTableOutput("gtg_table_KJ"), 
                                                                    style = "font-size: 80%"))))
                                    )
                        )
               )
      ...
      output[[paste0("tab_moveout_", 
                     paste0(unlist(str_extract_all(input$Dash, "[:upper:]")), collapse = ""))]] <- renderDataTable({
                       allGames %>%
                         ...
                         'Strike %' = 100*round(Balls/Count, 3),
                         Swings = sum(PitchCall %in% c("FoulBall", "InPlay", "StrikeSwinging")),
                         'Swing %' = 100*round(Swings/Count, 3),
                         Fouls = sum(PitchCall == 'FoulBall'),
                          'Foul %' = 100*round(Fouls/Count, 3),
                         SwStr = sum(PitchCall == "StrikeSwinging"),
                         'Whiff %' = 100*round(SwStr/Swings, 3),
                         'BIP %' = (100*sum(PlayResult %in% c("Single", "Double", "Triple", "Out", 
                                                              "Error", "FieldersChoice") | 
                                              (TaggedHitType == "Bunt" & PlayResult == "Sacrifice"))/Count) %>% round(1),
                         "GB %" = (100*sum(TaggedHitType == "GroundBall")/Count) %>% round(1),
                         "FB %" = (100*sum(TaggedHitType == "FlyBall")/Count) %>% round(1),
                         "GB %" = (100*sum(TaggedHitType == "GroundBall")/Count) %>% round(1),
                         "LD %" = (100*sum(TaggedHitType == "LineDrive")/Count) %>% round(1),
                         HR = sum(PlayResult == "HomeRun"),
                         BABIP = (sum(PlayResult %in% c("Single", "Double", "Triple"))/
                           sum(PlayResult %in% c("Single", "Double", "Triple", "Out", "Error", "FieldersChoice") | 
                                 (TaggedHitType == "Bunt" & PlayResult == "Sacrifice"))) %>% round(3),
                         ...
                         select(-c(Strikes, Balls, Swings, Fouls, SwStr))
                       })
      })
    # Usage table
    observe({
