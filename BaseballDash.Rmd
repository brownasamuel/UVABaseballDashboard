...
find_stats <- function(name){
  pit_stats %>%
    ...
    mutate("K/9" = 9*as.numeric(so)/as.numeric(ip),
           "BB/9" = 9*as.numeric(bb)/as.numeric(ip),
           "HR/9" = 9*as.numeric(hr)/as.numeric(ip))
}
```
  ...
  navbarPage(
    ...
    navbarMenu(title = "Pitcher Profiles",
               ...
               tabPanel(title = "Connelly Early", value = "Connelly Early",
                        fluidRow(
                          column(2,
                                 div(imageOutput("pic_CE"), style = "height:294px")),
                          column(9,
                                 div(dataTableOutput("stats_CE"), style = "font-size: 95%"))),
                        # Put in options for what to look at
                        tabsetPanel(tabPanel(title = "In-Game Durability", value = "durab",
                                             fluidRow(column(2, uiOutput("ptypeCEdurab")),
                                                      column(6, plotOutput("pc_graph_CE")),
                                                      column(4, 
                                                             div(dataTableOutput("pc_table_CE"), 
                                                                 style = "font-size: 80%")))),
                                    tabPanel(title = "Game-to-game Performance",
                                             fluidRow(column(2, uiOutput("ptypeCEgtg"), uiOutput("metricCEgtg")),
                                                      column(6, plotOutput("gtg_graph_CE")),
                                                      column(4, 
                                                             div(dataTableOutput("gtg_table_CE"), 
                                                                 style = "font-size: 80%")))),
                                    tabPanel(title = "Zone Profile",
                                             fluidRow(column(2, uiOutput("ptypeCEzp"), uiOutput("bhandCEzp")),
                                                      column(6, plotOutput("zp_plot_CE")),
                                                      column(4))),
                                    tabPanel(title = "Pitch Profiles",
                                             # Add in pie charts to pitch profiles plus table of results
                                             fluidRow(column(2, uiOutput("xaxisCEpp"), uiOutput("yaxisCEpp")),
                                                      column(6, plotOutput("pp_plot_CE")),
                                                      column(4,
                                                             div(dataTableOutput("pp_table_CE"),
                                                                 style = "font_size: 80%")))),
                                    tabPanel(title = "Results by Count"
                                             # Add in location plots and tables of results 
                                             )
                                    )
                                    # Add in brooks baseball tables
                        ),
               tabPanel(title = "Kevin Jaxel", value = "Kevin Jaxel",
                        fluidRow(
                          column(2,
                                 div(imageOutput("pic_KJ"), style = "height:294px")),
                          column(9,
                                 div(dataTableOutput("stats_KJ"), style = "font-size: 80%"))),
                        tabsetPanel(tabPanel(title = "In-Game Durability", value = "durab",
                                             fluidRow(column(2, uiOutput("ptypeKJdurab")),
                                                      column(6, plotOutput("pc_graph_KJ")),
                                                      column(4, 
                                                             div(dataTableOutput("pc_table_KJ"), 
                                                                 style = "font-size: 80%")))),
                                    tabPanel(title = "Game-to-game Performance",
                                             fluidRow(column(2, uiOutput("ptypeKJgtg"), uiOutput("metricKJgtg")),
                                                      column(6, plotOutput("gtg_graph_KJ")),
                                                      column(4, 
                                                             div(dataTableOutput("gtg_table_KJ"), 
                                                                 style = "font-size: 80%")))),
                                    tabPanel(title = "Zone Profile",
                                             fluidRow(column(2, uiOutput("ptypeKJzp"), uiOutput("bhandKJzp")),
                                                      column(6, plotOutput("zp_plot_KJ")),
                                                      column(4))),
                                    tabPanel(title = "Pitch Profiles",
                                             fluidRow(column(2, uiOutput("xaxisKJpp"), uiOutput("yaxisKJpp")),
                                                      column(6, plotOutput("pp_plot_KJ")),
                                                      column(4,
                                                             div(dataTableOutput("pp_table_KJ"),
                                                                 style = "font_size: 80%"))))
                                    )
                        )
               )
...
server = function(input, output) {
  ...
  observe({
    ...
    ### Inner tab 3- Pitch Profiles
    # Select x-axis variable
    observe({
      output[[
        paste0("xaxis", paste0(unlist(str_extract_all(input$Dash, "[:upper:]")), collapse = ""), "pp")]] <- renderUI({
    ...
    observe({
      output[[
        paste0("yaxis", paste0(unlist(str_extract_all(input$Dash, "[:upper:]")), collapse = ""), "pp")]] <- renderUI({
    ...
    # Make pitch graph
    observe({
      output[[paste0("pp_plot_", paste0(unlist(str_extract_all(input$Dash, "[:upper:]")), collapse = ""))]] <- 
        renderPlot({
          allGames %>%
            ...
            filter(Pitcher == sub("([^ ]*) ([^ ]*)", "\\2, \\1", "Connelly Early")) %>%
    ...
    # Make pitch profile table
    observe({
      output[[paste0("pp_table_", 
                     paste0(unlist(str_extract_all(input$Dash, "[:upper:]")), collapse = ""))]] <- renderDataTable({
                       allGames %>%
                         filter(Pitcher == sub("([^ ]*) ([^ ]*)", "\\2, \\1", input$Dash)) %>%
                         ...
                         summarize(Velocity = round(mean(RelSpeed, na.rm = T), 1),
                                   ...
                                   "Vertical Release Pint" = round(mean(RelHeight, na.rm = T), 1)) %>%
